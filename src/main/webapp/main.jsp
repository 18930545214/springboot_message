...
<link type="text/css" rel="stylesheet" href="css/sms.css" />
	...
	<script>
		...
		//查询短消息的函数
		function showMessage(pageNo, pageSize) {
			//发送ajax请求
			$.get("${pageContext.request.contextPath}/queryAllMessage/"+pageNo+"/"+pageSize, function (data) {
				//此代码要使用jquery进行拼接
				...
				for (var i = 0; i < data.list.length; i++) {
					...
					var $em2 = $("<em><a href='${pageContext.request.contextPath}/queryAllUser/" + msg.sendId + "'>回信</a></em>");
					var $em3 = $("<em><a href='javascript:deleteMessageById(" + msg.id + ")'>删除</em>");
					var $p = $("<p></p>");
				...
				$a = $("#btns > a");
				$a.eq(0).attr("href", "javascript:showMessage('" + 1 + "','" + pageSize + "')");
				$a.eq(1).attr("href", "javascript:showMessage('" + data.prePage + "','" + pageSize + "')");
				$a.eq(2).attr("href", "javascript:showMessage('" + data.nextPage + "','" + pageSize + "')");
				$a.eq(3).attr("href", "javascript:showMessage('" + data.pages + "','" + pageSize + "')");
		...
		function showMessageById(id) {
			window.location = "${pageContext.request.contextPath}/queryMessageById/" + id;
		}
		...
		function deleteMessageById(id) {
			...
			alertify.confirm("是否確定刪除?",
					function () {
						$.post("${pageContext.request.contextPath}/deleteMessageById/"+id, {_method:"delete"}, function (data) {
							if(data=="success"){
